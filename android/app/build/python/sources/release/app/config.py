...
#   * Android (Chaquopy)        -> <app private files>/FinWiseData
#   * source tree               -> <FinWise root>/FinWiseData
# ---------------------------------------------------------------------------
if sys.platform == "android":
    _base_dir = os.getenv("FINWISE_DATA_DIR") or os.getenv("HOME") or os.path.expanduser("~")
    _base_dir = Path(_base_dir) / "FinWiseData"
else:
    _base_dir = None
...
# Security -------------------------------------------------------------------
# Persist a generated secret so JWT tokens stay valid across restarts.
